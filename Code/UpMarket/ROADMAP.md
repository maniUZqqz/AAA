...
9. [**سیستم‌های جدید — ۱۸ سیستم**](#۹-سیستمهای-جدید)
10. [**تصمیم‌های باز**](#۱۰-تصمیمهای-باز)
11. [**رودمپ دوره‌ی دوم (فاز ۰.۵ و ۱۸ تا ۳۰)**](#۱۱-رودمپ-دورهی-دوم)
...
> **⚠️ به‌روزرسانی ۲۰۲۶/۰۹/۱۲ — لانچر یکی شد:** `start-v1.bat` و `start-v2.bat` حذف
> شدند و جایشان یک `start.bat` نشست که **بک‌اند + داشبورد + سایت عمومی** را با هم
> بالا می‌آورد. مسیر ComfyUI دیگر هاردکد نیست (خودکار پیدا می‌شود)، و با گذاشتن
> `AI_API_BASE_URL` در `backend/.env` متن و بینایی از API می‌آیند و Ollama/ComfyUI
> اصلاً اجرا نمی‌شوند — یعنی محصول روی سیستم بدون GPU هم کار می‌کند.
> حالت فعلی را `python manage.py aimode` می‌گوید.
...
#### نقص ۱۵ — راهی برای بازگشتن به حساب وجود نداشت

هیچ‌جا «رمزم را فراموش کردم» نبود — نه در سایت، نه در پنل، نه در API. بند ۲۲ سند
`سایت عمومی.md` چهار صفحه‌ی احراز هویت را فهرست کرده بود و دو تای آخرش
(`forgot-password` و `reset-password`) اصلاً به آن‌ها فکر نشده بود.

تا اولین مشتری واقعی، این هزینه‌ای ندارد. **از روز اول فروش** یعنی هر کسی رمزش را
گم کند باید شخصاً به یکی از ما پیام بدهد، هر ساعتی از شبانه‌روز که باشد.

**و ریشه‌ی مشکل عمیق‌تر بود:** `email` در ثبت‌نام **اختیاری** بود. یعنی حتی با ساختن
بازیابی رمز، کاربری که بدون ایمیل ثبت‌نام کرده بود **هیچ راه بازگشتی** نداشت.
`User.email` جنگو هم `unique` نیست، پس دو حساب با یک ایمیل، «لینک را کجا بفرستم؟»
را بی‌جواب می‌گذاشت.

**راه‌حل:** ساخته شد (پایین) · **فاز:** ۱۹
...
**✅ رفع‌شده در ۱۴۰۵/۰۶/۲۱:** نقص ۱۲ (`?plan=` دور ریخته می‌شد) و نقص ۱۳ (صفحه‌ی
تماس فرم نداشت) هر دو حل شدند — اپ جدید `apps/marketing` با مدل `Lead`، اندپوینت
عمومی، صندوق ورودی در جنگو ادمین، و `pendingPlan` در فرانت که انتخاب پکیج را تا
ساخت فروشگاه نگه می‌دارد. ۱۰ تست جدید.
...
### ۹.۱۲ — هوش فروشگاه: فروش، موجودی و سود واقعی

امروز `apps/analytics` فقط می‌شمارد. این بخش آن را به **علم داده‌ی فروش** تبدیل می‌کند.

#### چیزی که کم است: قیمت تمام‌شده

مدل `Product` قیمت فروش دارد ولی **قیمت تمام‌شده ندارد**. بدون آن هیچ محاسبه‌ی سودی ممکن نیست — فقط گردش مالی محاسبه می‌شود، نه سود.

```text
Selling Price   ← الان هست
Cost Price      ← باید اضافه شود
────────────────────────────
Real Profit     ← تازه آن‌وقت ممکن می‌شود
```
...
| تحلیل | چه می‌گوید |
|---|---|
| پرفروش‌ترین | کدام محصول بیشترین **تعداد** را فروخته |
| سودآورترین | کدام محصول بیشترین **سود** را ساخته — ممکن است همان نباشد |
| نمودار فروش | روند فروش هر محصول در زمان |
| نمودار موجودی | چه چیزی رو به اتمام است، چه چیزی خوابیده |
| مقایسه‌ی محصولات | کنار هم، بر اساس فروش و سود و گردش |

> **نکته‌ی مهم:** «پرفروش‌ترین» و «سودآورترین» معمولاً یکی نیستند. محصولی که زیاد می‌فروشد ممکن است حاشیه‌ی نازکی داشته باشد. فروشگاه‌دار باید هر دو را ببیند.

#### تغذیه به بقیه‌ی سیستم

این داده نباید در یک صفحه بماند:

```text
Sales Data → کدام محصول محتوا لازم دارد؟
           → کدام محصول را ایجنت فروش بیشتر پیشنهاد بدهد؟
           → کدام محصول در کمپین بعدی برود؟
```

---

### ۹.۱۳ — ترندها و اتصالشان به تولید محتوا

#### جمع‌آوری

ترندهای **ایرانی** و **خارجی** از شبکه‌های اجتماعی و منابع منتشرکننده‌ی ترند.

برای هر ترند: موضوع چیست، چه نوع محتوایی حولش ساخته می‌شود، چقدر داغ است، و از کِی.

#### پیشنهاد — نه فهرست خام

فهرست ترند به‌تنهایی بی‌فایده است. سیستم باید ترند را با فروشگاه جوش بدهد:
...
Audience
     ↓
«برای محصول X، این ویدیو را با این زاویه بساز»
```

#### اتصال مستقیم

کاربر باید از همین صفحه یک ترند را انتخاب کند و **مستقیم وارد تولید محتوا شود** — اطلاعات ترند همراهش منتقل شود:

```text
Trend → Select Trend → Select Product → Content Generation
```

> **قانون صداقت:** ترند باید از منبع واقعی بیاید با تاریخ و لینک — نه اینکه مدل از خودش «ترند» دربیاورد. همان قانونی که برای تحلیل رقبا داریم.

---

### ۹.۱۴ — دو حالت تولید: ساده و پیشرفته

#### حالت ساده

از چیزی که **از قبل داریم** استفاده می‌کند:

```text
Product Information + Additional Prompt + Optional Trend → Generate
```

کاربر چیز جدیدی نمی‌دهد. سریع است و برای بیشتر موارد کافی.

#### حالت پیشرفته

سیستم **تشخیص می‌دهد چه چیزی کم دارد و می‌پرسد**:
...
AI Analysis
     ↓
«برای این خروجی، عکس محصول از زاویه‌ی روبه‌رو لازم دارم»
     ↓
User Provides Image / Video / Info
     ↓
Generation → Caption → Advanced Editing → Final
```

تفاوت اصلی: **پیشرفته ورودی بیشتری می‌خواهد تا خروجی خاص‌تر و باکیفیت‌تر بدهد.**

#### دو نوع خروجی تصویر — صریح و جدا

| حالت | برای چه | قانون |
|---|---|---|
| **پوستر / بیلبورد** | کمپین، تبلیغ، پیشنهاد ویژه | AI طرح می‌سازد، **کد متن را می‌نشاند** |
| **نمایش محصول** | پست، استوری، معرفی | **محصول واقعی باید حفظ شود** |

در حالت نمایش محصول، AI اجازه دارد مدل، محیط، نور، پس‌زمینه و ترکیب‌بندی را عوض کند — **ولی خود محصول را نه**.

---

### ۹.۱۵ — تنظیمات ایجنت فروش

ایجنت نباید برای همه‌ی فروشگاه‌ها یک‌جور حرف بزند.

#### لحن

رسمی · دوستانه · حرفه‌ای · صمیمی · یا لحن سفارشی خودِ برند.

#### قوانین فروش

فروشگاه‌دار باید بتواند صریح بگوید:
...
کِی تخفیف بدهد و کِی نه
```

این تنظیمات باید **واقعاً روی رفتار مدل اعمال شوند**، نه اینکه فقط ذخیره شوند.

> **قانون سخت که تغییر نمی‌کند:** هیچ تنظیمی نمی‌تواند دو قانون پایه را بشکند — قیمت و موجودی از دیتابیس می‌آید، و تأیید پرداخت با انسان است.

---

### ۹.۱۶ — پلتفرم API عمومی

فروشگاه‌دار باید بتواند قابلیت‌های آپ‌مارکت را **داخل سیستم خودش** استفاده کند.

| API | چه می‌دهد |
|---|---|
| **Sales Admin** | ایجنت فروش — جواب دادن، پیشنهاد، ثبت سفارش |
| **Text & Caption** | متن فروش و کپشن |
| **Advertising Video** | ویدیوی تبلیغاتی |
| **Advertising Poster** | پوستر با متن درج‌شده |
| **Product Showcase** | عکس محصول در صحنه |

هر API لازم دارد: احراز هویت، سهمیه و محدودیت نرخ، مستندات، نمونه‌کد، و **شمارش مصرف در همان سیستم اعتبار** ([۹.۹](#۹۹--اشتراک-اعتبار-و-تضمین-کیفیت)).

> این همان سیستمی است که خودمان استفاده می‌کنیم — پس اگر داخلاً درست ساخته شده باشد، بیرونی‌کردنش عمدتاً کار احراز هویت و سهمیه است، نه بازنویسی.

---

### ۹.۱۷ — بازاریابی تلفنی

> **وضعیت: Coming Soon.** کد نوشته نمی‌شود، ولی معماری از الان مشخص است تا بقیه‌ی سیستم جوری ساخته شود که بعداً جا بدهد.
...
شماره یا نام شرکت
     ↓
Customer / Company Research     ← پیدا کردن پروفایل و راه ارتباطی
     ↓
ارتباط اولیه                    ← پیام یا تیکت
     ↓
Mobile Call Assistant          ← موقع تماس، پروفایل را نشان می‌دهد
     ↓
Real-Time Sales Assistant      ← در لحظه پیشنهاد می‌دهد چه بگوید
     ↓
Call Recording & Analysis
     ↓
Post-Call Analysis             ← استخراج اطلاعات → به‌روزرسانی پروفایل
```

#### تحقیق شرکت

از نام یا سایت شرکت: راه‌های ارتباطی، HR، مارکتینگ، فروش، و **افراد تصمیم‌گیرنده**.

#### ⚠️ دو قید که قبل از هر خط کد باید حل شوند

**۱. قانونی بودن ضبط تماس.** ضبط مکالمه بدون رضایت طرف مقابل در بسیاری از حوزه‌های قضایی جرم است. این قابلیت باید با رضایت صریح و اعلام شفاف طراحی شود، نه به‌عنوان قابلیت پنهان.

**۲. جمع‌آوری اطلاعات شخصی.** «شماره را بده تا در تلگرام و شبکه‌ها دنبالش بگردیم» از نظر حریم خصوصی حساس است. باید مشخص شود چه داده‌ای، از چه منبع مجازی، با چه مجوزی، و چقدر نگه داشته می‌شود.

> این قابلیت تا وقتی این دو روشن نشده‌اند نباید وعده داده شود — نه در سایت، نه به مشتری.

---

### ۹.۱۸ — سایت عمومی

مرجع کامل: [`../../سایت عمومی.md`](../../سایت%20عمومی.md)

#### وضعیت امروز در برابر مشخصات

| | دارد | می‌خواهد |
|---|---|---|
| روت پایه | **۱۱** | ~۲۵ |
| دوزبانه | ❌ فقط متادیتای `fa_IR` | فارسی + انگلیسی با `hreflang` |
| دارک/لایت | ✅ هست | ✅ |
| فرم تماس | ❌ استاتیک | متصل به بک‌اند، ثبت Lead |
| `?plan=` | ❌ دور ریخته می‌شود | تا انتهای ثبت‌نام حفظ شود |

#### چه روت‌هایی کم است

```text
/solutions · /solutions/[slug]        ← صنعت‌محور، برای سئو
/use-cases · /use-cases/[slug]        ← «با آپ‌مارکت چه می‌توانم بکنم»
/case-studies · /case-studies/[slug]
/features/[slug]                      ← صفحه‌ی اختصاصی هر قابلیت
/blog/category/[slug]
/customers · /testimonials
/resources (guides · templates · checklists · ebooks)
/tools/[slug]                         ← ابزار رایگان، کانال جذب
/compare/[slug]
/login · /register · /forgot-password · /reset-password
/privacy-policy · /terms · /cookie-policy · /refund-policy · /acceptable-use
```

#### اصل معماری

**تمپلیت بساز، نه صفحه.** هدف این نیست که روز اول ۱۰۰ صفحه دستی بسازیم؛ هدف این است که معماری بتواند بعداً ۵۰۰ صفحه بدهد بدون تغییر اساسی در فرانت.

#### قانون‌هایی که نباید شکسته شوند

- قیمت **هاردکد نشود** — از بک‌اند بیاید تا سایت و محصول هرگز اختلاف نداشته باشند.
- فرم تماس **واقعی** باشد، نه `mailto:` و نه فرم ظاهری.
- تگ‌های کم‌محتوا **ایندکس نشوند**.
- نمونه‌کار ساختگی به‌عنوان مشتری واقعی نمایش داده نشود.
- ادعای غیرقابل‌اثبات درباره‌ی افزایش فروش نوشته نشود.
- Schema فقط وقتی استفاده شود که با محتوای واقعی صفحه بخواند.
...
> **منابع این دوره:** بازبینی کد (بخش ۷)، [`ایده‌ها پنل.md`](../../ایده‌ها%20پنل.md) و
> [`سایت عمومی.md`](../../سایت%20عمومی.md). فازهای ۲۷ تا ۳۰ از دو سند دوم آمده‌اند.
...
> 🔧 **زیرساخت اندازه‌گیری ساخته شد** (روی این سیستم بدون کارت گرافیک):
> بنچمارک را نمی‌شود اینجا اجرا کرد، ولی می‌شود کاری کرد که **نتیجه‌اش
> محصولِ جانبی استفاده‌ی معمولی باشد نه تمرینی که کسی باید یادش بماند
> یادداشت کند.**
>
> - `services/hardware.py` — GPU/VRAM/RAM را می‌خواند، و هر چیزی که نمی‌داند
>   `None` می‌گذارد نه حدس.
> - `apps/jobs/metrics.py` — هر قطعه‌ی ویدیو که رندر شود، خودش ثبت می‌شود:
>   زمان دیوار، ثانیه‌های خروجی، VRAM، زمان بار کردن مدل، و **موفق یا ناموفق**.
> - `python manage.py benchmark` — گزارش، و `--json` دقیقاً بلوک `render` را
>   برای `data.json` چاپ می‌کند تا کسی اعشار را دستی تایپ نکند.
>
> **دو تصمیم که عدد را عوض می‌کنند:**
>
> ۱. **میانه، نه میانگین.** زمان رندر دنباله‌ی راست بلندی دارد (بار شدن مدل،
>    throttle حرارتی)، و میانگینی که یک اجرای ۴۵ دقیقه‌ای بالا کشیده، ماشینی
>    را توصیف می‌کند که وجود ندارد.
> ۲. **شکست‌ها هم ثبت می‌شوند.** خطی که ۴ دقیقه طول می‌کشد ولی یک‌سوم مواقع
>    شکست می‌خورد، در عمل **۶ دقیقه** برای هر خروجی قابل‌استفاده هزینه دارد.
>    `benchmark --json` عدد تصحیح‌شده با نرخ شکست را جدا می‌دهد
>    (`video_gpu_min_per_second_effective`) — همان که مدل ظرفیت باید بگیرد،
>    نه میانه‌ی موفق‌ها که به خط تولید خوش‌بین است.
>
> ماشین‌ها هرگز با هم میانگین نمی‌شوند: هر ردیف اثر انگشت سخت‌افزار دارد، چون
> میانگین یک ۳۰۶۰ و یک ۴۰۹۰ هیچ‌کدام را توصیف نمی‌کند.
>
> **پس کاری که روی سیستم اصلی مانده:** یک تیزر واقعی end-to-end بسازید. گزارش
> خودش ساخته می‌شود.
...
- [x] **PostgreSQL برای Production** ✅ — با `DATABASE_URL`؛ خالی بگذاری SQLite می‌ماند.
  اسکیم غیر از postgres رد می‌شود، رمز URL-encoded درست decode می‌شود،
  `CONN_MAX_AGE` تا اتصال به‌ازای هر درخواست باز نشود. SQLite دِو هم WAL و
  busy_timeout گرفت تا کمی شبیه production رفتار کند.
- [x] **Dockerfile + docker-compose + Nginx** ✅ — ۹ سرویس:
  db · redis · migrate · backend · worker-ai · worker-gpu · website · nginx · backup
  - ایمیج‌ها multi-stage و **non-root**؛ کامپایلر در لایه‌ی runtime نمی‌ماند.
  - `migrate` سرویس جداست، نه entrypoint — وگرنه هر replica هم‌زمان روی یک
    دیتابیس migrate می‌زند.
  - `worker-gpu` با `concurrency=1` چون یک کارت بیشتر نیست؛ `worker-ai` جداست
    تا رندر ۴۰ دقیقه‌ای کپشن‌ها را پشت خودش نگه ندارد.
  - redis با `appendonly` تا ری‌استارت، صف رندرها را بی‌صدا دور نریزد.
  - nginx یک مبدأ برای همه‌چیز می‌سازد و `X-Forwarded-For` واقعی را پاس می‌دهد
    (اندپوینت لید برای تشخیص سوءاستفاده رویش حساب کرده).
- [x] **Backup + مستند Deployment** ✅ — دامپ شبانه روی هاست (نه داخل ولوم، تا
  بازیابی به زنده بودن کانتینر وابسته نباشد)، اول `.partial` بعد rename چون
  فایل نیمه‌نوشته با نام واقعی از نبودن بکاپ بدتر است.
- [x] **گاردهای production** ✅ — بالا نمی‌آید اگر: `SECRET_KEY` پیش‌فرض باشد،
  `ALLOWED_HOSTS=*` باشد. و اگر `DATABASE_URL` خالی باشد هشدار SQLite می‌دهد.
- [x] **AI Gateway** ✅ — `services/ai/gateway.py`، تنها درِ بین منطق کسب‌وکار و
  هر سرویس هوش مصنوعی.
  - `factory.text_provider()` بدون فروشگاه **حذف شد**، نه اینکه منسوخ اعلام شود.
    همان تابع بود که اجازه می‌داد یک سوییچ پلتفرم‌واید، دادهٔ یک فروشگاه را
    جابه‌جا کند بی‌آنکه صاحبش بداند. حالا امضای گیت‌وی `store` را **اجباری**
    می‌گیرد: تماسی که نمی‌تواند بگوید دادهٔ چه کسی را حمل می‌کند، حق زدنش را ندارد.
  - قانون با **تست** نگه داشته می‌شود نه با یادداشت: `tests_boundary.py` درخت
    `apps/` را AST-پارس می‌کند و اگر ماژولی مستقیم `ollama` یا `openai_compat`
    یا `factory` را ایمپورت کند، بیلد قرمز می‌شود. دو صفحهٔ ادمینِ خودِ
    سرویس‌دهنده‌ها با دلیل استثنا شده‌اند.
- [x] **Data Policy Layer** ✅ — `StoreAIPolicy` per-store با سه حالت.
  - `Local Only` یعنی **کار با پیام روشن fail می‌شود**، نه اینکه بی‌صدا به API برود.
  - دو دروازهٔ مستقل برای دادهٔ مشتری: اپراتور باید سرویس را برای نوع دادهٔ
    `CUSTOMER` مجاز کرده باشد **و** مالک فروشگاه صریحاً اجازه داده باشد. پیام
    خصوصی مشتری، دادهٔ خودِ اوست نه فروشگاه؛ پیش‌فرض خاموش است.
  - سرویس بیرونی که تیک «تأییدشده» بخورد، تا نگوید داده کجا پردازش و چقدر
    نگه‌داری می‌شود ذخیره نمی‌شود — وگرنه «تأیید» فقط یک مهر لاستیکی است.
  - کارت پنل (`DataPolicyCard`) هم تنظیم را می‌دهد هم **می‌گوید چه چیزی با آن
    از کار می‌افتد**؛ تنظیمی که بگوید «فقط لوکال» ولی نگوید صداگذاری قطع می‌شود، تله است.
- [x] **اصلاح ادعای Privacy** ✅ — «داده‌ی شما پیش ما می‌ماند، نه سرویس خارجی»
  با کد نمی‌خواند: edge-tts متن نریشن را به مایکروسافت می‌فرستد و تحقیق رقبا
  دیجی‌کالا/ترب را می‌خواند. متن سایت و `data.json` هر دو به همان چیزی که
  واقعاً اتفاق می‌افتد اصلاح شدند. `get_tts_provider` هم حالا سیاست فروشگاه را
  چک می‌کند و برای فروشگاه «فقط لوکال» صدا **تولید نمی‌کند**.
- [ ] Object Storage جدا از دیتابیس — فعلاً ولوم؛ لازم می‌شود وقتی موتور تدوین (فاز ۲۳) بیاید
- [ ] Monitoring
...
> **🐛 سه ناهماهنگی که فقط موقع نوشتن دیپلوی پیدا شدند** — هیچ‌کدام در تست یا بیلد دیده نمی‌شدند:
> ۱. کامپوز `CELERY_BROKER_URL` می‌داد ولی settings فقط `REDIS_URL` می‌خواند —
>    ورکرهای دیپلوی‌شده بی‌صدا به localhost برمی‌گشتند و بروکری پیدا نمی‌کردند.
> ۲. `CSRF_TRUSTED_ORIGINS` اصلاً تنظیم نمی‌شد — پشت پروکسی TLS، لاگین ادمین ۴۰۳
>    می‌داد و شبیه رمز اشتباه به نظر می‌رسید. `SECURE_PROXY_SSL_HEADER` هم اضافه شد.
> ۳. بیلد standalone سایت، خروجی را در `.next/standalone/Desktop/upM/.../server.js`
>    می‌گذاشت چون Next ریشه‌ی ردیابی را از مسیر مطلق حدس می‌زد —
>    `COPY` داکرفایل چیزی پیدا نمی‌کرد. با `outputFileTracingRoot` پین شد.
>
> **⚠️ آنچه تأیید نشده:** داکر روی این سیستم نصب نیست، پس **ادعا نمی‌کنم استک
> بالا می‌آید**. تأییدشده: YAML معتبر، وابستگی سرویس‌ها درست، تنظیمات با
> `DATABASE_URL` واقعی PostgreSQL می‌سازد، گاردها واقعاً جلوی بالا آمدن را
> می‌گیرند، و بیلد standalone اجرا شد و روی پورت ۲۰۰ داد.
> اولین `docker compose up` باید روی سرور انجام شود.

> 🐛 **باگی که فقط اجرای واقعیِ DoD پیدا کرد — و مهم‌تر از خودِ فاز است:**
> برای اثبات DoD یک اجرای واقعی گذاشتم: فروشگاهی با سیاست `Local Only` و تنها
> یک سرویس بیرونی. حالت `Local Only` درست fail شد، ولی حالت `Hybrid` هم fail
> شد — با این خطا:
>
> ```
> OpenAICompatProvider.generate_json() takes 2 positional arguments but 3 were given
> ```
>
> یعنی **مسیر «متن را به API بدهیم» هیچ‌وقت کار نمی‌کرد.** `_single_call` مدل را
> positional می‌دهد و `(parsed, raw)` را unpack می‌کند؛ نسخهٔ API مدل را
> keyword-only می‌گرفت و فقط یک dict برمی‌گرداند. هیچ تستی نگرفته بود چون هر
> تستِ آن لایه، provider را mock می‌کند — mock هر امضایی را قبول می‌کند.
>
> **چرا این مهم است:** [۸.۴](#۸۴--ai-gateway) همین مسیر را راه‌حل مشکل ظرفیت
> GPU می‌داند. اگر روز شلوغی می‌خواستیم متن را به API منتقل کنیم تا GPU فقط
> برای رندر بماند، با TypeError روبه‌رو می‌شدیم — نه در تست، که در production.
>
> امضا و نوع خروجی هم‌تراز شد و `ProviderContractTests` اضافه شد که امضای دو
> provider را با `inspect` مقایسه می‌کند و یک بار هم دقیقاً همان تماسی را که
> `_single_call` می‌زند اجرا می‌کند. اجرای دوباره: `Hybrid` واقعاً به شبکه رسید
> (DNS دامنهٔ ساختگی شکست) و `Local Only` قبل از هر تماس شبکه‌ای متوقف شد.
>
> ✅ **۲۹۷ تست بک‌اند سبز** (۲۶۳ → ۲۹۷).
...
- [x] **Payment Gateway با معماری Provider-Based** ✅ — `apps/billing/payments/`
  - سه درگاه: **زرین‌پال** (کامل و واقعی، فقط merchant id لازم دارد)، **واریز بانکی**
    (همان کاری که امروز می‌کنید، ولی داخل سیستم تا آشتی‌دادن حساب‌ها یک جدول باشد نه دو تا)،
    و **درگاه آزمایشی** که در production خودش را غیرفعال می‌کند.
  - سه قانون که خم نمی‌شوند: **هیچ‌چیز با حرف مرورگر فعال نمی‌شود** (تأیید فقط
    سرور-به-سرور)، **مبلغ از ردیف پلن خوانده می‌شود نه از بدنه‌ی درخواست**، و
    **تأیید idempotent است** (callback دوباره، اشتراک را دوبار تمدید نمی‌کند).
  - تبدیل تومان↔ریال فقط داخل همان درگاهی که لازم دارد — خطای ۱۰ برابری در
    صورتحساب فقط روی کارت یک مشتری واقعی دیده می‌شود.
- [x] **Grace / Invoice / History / Refund / Audit Log** ✅
  - مهلت ارفاقی (پیش‌فرض ۳ روز) — کارت منقضی نباید فوراً سرویس را قطع کند.
    ولی اشتراک آزمایشیِ تمام‌شده مهلت نمی‌گیرد.
  - `Payment` هم فاکتور است هم تاریخچه‌ی تراکنش؛ مدل Invoice جدا یعنی دو منبع
    حقیقت برای یک پول، و آن دو از هم جدا می‌افتند.
  - تلاش‌های ناموفق هم نگه داشته می‌شوند: «چرا این مشتری مشترک نشد» فقط از
    ردیفی جواب می‌گیرد که می‌گوید درگاه ردش کرد.
  - `BillingEvent` فقط-افزودنی: هر تغییر اشتراک با زمان، عامل و مقدار قبلی.
  - بازگشت وجه **تصمیم را ثبت می‌کند نه اینکه وانمود کند پول را جابه‌جا کرده** —
    هیچ درگاه ایرانی API قابل‌اتکای refund ندارد و دکمه‌ای که ظاهراً پول برگردانده
    از نبودنش بدتر است.
- [x] **Credit Architecture** ✅ — `apps/billing/credits.py`
  - `reserve/commit/release` فقط حالتِ «کار کرَش کرد» را می‌پوشاند. این ماژول
    حالت سخت‌تر را می‌پوشاند: رندر **موفق شد**، اعتبار مصرف شد، و خروجی
    بی‌استفاده است. بدون این، فروشگاهی که یک پوستر را چهار بار می‌سازد چهار بار
    پول می‌دهد و نتیجه می‌گیرد محصول گران است — نه اینکه مدل آن روز بد بود.
  - `REFUNDED` از `RELEASED` **جدا** است. آزادشده = کار کرَش کرد و چیزی تولید
    نشد؛ برگشت‌خورده = تولید شد، پول گرفتیم، و خروجی بی‌استفاده بود. یکی‌کردنشان
    جواب «چقدر بابت خروجی بد برمی‌گردانیم؟» را غلط می‌دهد — و آن عدد همان است
    که می‌گوید محصول کار می‌کند یا نه.
  - **تضمین کیفیت** ([۹.۹](#۹۹--اشتراک-اعتبار-و-تضمین-کیفیت) ردیف C) در کد است نه در متن تبلیغاتی:
    بعد از `QUALITY_MAX_ATTEMPTS` تلاش، تولید رایگان می‌شود و **اعتبار کل
    زنجیره** برمی‌گردد.
  - `source` از الان `CUSTOMER` / `AUTO` دارد چون قاضی خودکار (Vision QC، فاز ۲۱)
    ساخته نشده. امروز هر رأی از فروشگاه‌دار می‌آید؛ فردا QC روی همان دفتر
    می‌نویسد و جای دیگری عوض نمی‌شود.
- [x] Registration با حفظ `?plan=` تا انتهای مسیر ✅ (فاز ۲۷.۴)
- [x] **Onboarding** ✅ — `apps/stores/onboarding.py`
  - تور نیست. هر قدم از دیتابیس جواب می‌گیرد، پس فروشگاهی که محصول دارد هرگز
    «اولین محصولت را اضافه کن» نمی‌بیند — و چک‌لیستی که یک‌بار دروغ بگوید،
    دیگر خوانده نمی‌شود.
  - قدم‌ها `blocked_by` دارند. تحلیل محصولی که عکس ندارد، فرستادن آدم به جایی
    است که نمی‌تواند تمامش کند؛ پس `next` قدم‌های قفل را رد می‌کند.
  - «تنظیمات ایجنت» با **ساخته شدن ردیف** تیک نمی‌خورد (ردیف با اولین خواندن
    ساخته می‌شود)، با **تغییر دادنش** تیک می‌خورد.
- [x] **Multi-user Store** ✅ — `apps/stores/access.py` + `Membership`
  - شش نقش با دو جدول جدا: چه چیزی را می‌تواند **عوض** کند و چه چیزی را
    **ببیند**. خواندن عمداً بازتر است — بازاریابی که کاتالوگ را نبیند نمی‌تواند
    برایش کمپین بنویسد. استثنا: صورتحساب و فهرست اعضا مال مالک است.
  - `Store.owner` ردیف عضویت **نیست**. مالکیت لنگر مستأجر است (کی پول می‌دهد،
    کی می‌تواند حذف کند)؛ اگر ردیف بود، می‌شد آخرین مالک را برداشت و یک
    فروشگاه پولی را بی‌صاحب کرد.
  - **۳۲ جای کد** `owner=request.user` می‌پرسیدند. همه به لایه‌ی دسترسی منتقل
    شدند و `tests_access.py` درخت `apps/` را می‌گردد و بیلد را قرمز می‌کند اگر
    کسی دوباره خودش بپرسد — یک جای جامانده، نقص ایزولاسیون مستأجر است نه
    قابلیتِ کم.
  - دو حالت خطا جدا ماندند: غریبه **۴۰۴** (وجود فروشگاه، دانستنی‌اش نیست)، عضوِ
    بی‌اجازه **۴۰۳** با نام نقشش (تا برود نقش بخواهد، نه اینکه باگ ثبت کند).
- [x] فرم تماس واقعی ✅ (فاز ۲۷.۴) — `POST /api/v1/leads/` با هانی‌پات و throttle
- [x] **بازیابی رمز عبور** ✅ ([نقص ۱۵](#نقص-۱۵--راهی-برای-بازگشتن-به-حساب-وجود-نداشت))
  — `apps/accounts/password_reset.py` + دو صفحه‌ی پنل
  - **ریشه اول درست شد:** `email` در ثبت‌نام **الزامی و یکتا** شد. بازیابی رمز
    برای کسی که ایمیل ندارد بی‌معنی است، و دو حساب با یک ایمیل «لینک را کجا
    بفرستم؟» را بی‌جواب می‌گذارد. فرم ثبت‌نام هم دیگر نمی‌گوید «(اختیاری)» و
    زیرش نوشته چرا لازم است.
  - **توکن ذخیره نمی‌شود.** `default_token_generator` جنگو آن را از هشِ رمز و
    آخرین ورود کاربر می‌سازد؛ پس جدولی برای جارو کردن نیست و — مهم‌تر — **با
    عوض شدن رمز خودش باطل می‌شود**. جدول توکن، هر ردیف فراموش‌شده‌اش یک کلید
    یدکی برای مغازه‌ی یک نفر است.
  - **جواب هرگز نمی‌گوید ایمیل ثبت شده یا نه.** یک کد وضعیت، یک متن، در هر دو
    حالت. اندپوینتی که جوابش فرق کند، ابزاری است برای فهمیدن اینکه کدام ایمیل
    مشتری شماست — و آن فهرست برای آدم نادرست ارزش پولی دارد.
  - **اگر ایمیل فرستاده نشد، همان را می‌گوید** (۵۰۳)، نه «لینک فرستادیم».
    مشتری‌ای که باور کند لینکی در راه است، منتظر می‌ماند جای اینکه کمک بخواهد.
  - **تأیید، کسی را وارد نمی‌کند.** جواب فقط «ثبت شد» است و هیچ توکنی ندارد.
    دادن نشست یعنی لینکِ لو رفته، **تصرف کامل حساب** است نه تغییر رمزی که
    صاحبش می‌بیند اتفاق افتاده.
  - لینک قبل از نمایش فرم **بررسی می‌شود**؛ گفتن «منقضی شده» بعد از اینکه کاربر
    دو بار رمز تایپ کرد، همان خردهْ‌آزاری است که آدم را به پیام دادن به
    پشتیبانی می‌رساند.
  - خطای لینک بد و uid بد **یک پیام** دارند: تفکیکشان به غریبه‌ای که لینک حدس
    زده می‌گوید کدام نیمه را درست زده.
  - نرخ ۵ درخواست در ساعت. هر درخواست یک ایمیل واقعی به کسی می‌فرستد که لزوماً
    خودش نخواسته؛ اندپوینت بی‌محدودیت، راهی است برای پر کردن صندوق یک
    فروشگاه‌دار از بیرون.
- [x] **تنظیمات ایجنت فروش** ✅ ([۹.۱۵](#۹۱۵--تنظیمات-ایجنت-فروش)) — `apps/stores/agent_settings.py`
  رودمپ گفته بود «باید **واقعاً روی رفتار مدل اعمال شوند**». پس سه لایه شد:
  ۱. **در پرامپت** — چیزی که باعث می‌شود مدل **معمولاً** رعایت کند.
  ۲. **بعد از جواب** — عبارت‌های `never_say` در خودِ متن جواب جست‌وجو می‌شوند،
     چون «معمولاً» قول نیست. جوابی که قانون را بشکند مثل JSON خراب رد می‌شود:
     مدل بعدی امتحان می‌شود، و اگر هیچ‌کدام جواب سالم ندادند، گفتگو به آدم
     می‌رسد. **نگفتن، از گفتنِ چیز ممنوع بهتر است.**
  ۳. **هیچ‌وقت** برای دو قانونی که خم نمی‌شوند — قیمت و موجودی از دیتابیس،
     تأیید پرداخت با آدم. قوانین فروشگاه **قبل** از این‌ها در پرامپت می‌آیند
     چون آخرین دستوری که مدل می‌خواند بیشترین وزن را دارد.
  - پیش‌فرض‌ها محافظه‌کارانه‌اند: بدون تخفیف، شکایت به آدم. ایجنتی که چون
    کسی تنظیمش نکرده تخفیف می‌دهد، هزینه‌ای است که فروشگاه قبول نکرده.

**DoD:** یک کاربر از صفحه‌ی قیمت سایت تا فروشگاه فعال با اشتراک پرداخت‌شده می‌رسد، **بدون هیچ دخالت دستی ادمین**.

> ✅ **DoD پرداخت تأیید شد** (۲۰۲۶/۰۹/۱۲، اجرای واقعی روی سرور):
> ثبت‌نام ← ساخت فروشگاه ← شروع پرداخت ← بازگشت از درگاه ← اشتراک **فعال**،
> فاکتور صادر، ردپای حسابرسی کامل. هیچ دخالت ادمینی لازم نشد.
> **۴۳ تست billing سبز** (۲۶۳ تست کل بک‌اند).
>
> 🐛 **دو باگ که تست‌ها و اجرای واقعی گرفتند:**
> ۱. شماره فاکتور از ساعت تا ثانیه ساخته می‌شد، پس دو پرداخت هم‌زمان تصادم
>    می‌کردند — و دو مشتری هم‌زمان، حالت عادی روز اول است نه لبه‌ی نادر.
>    حالا پیشوند تاریخ + پسوند تصادفی، با الفبای بدون کاراکتر مبهم چون این
>    شماره‌ها پای تلفن پشتیبانی خوانده می‌شوند.
> ۲. اولین پرداخت روی اشتراک آزمایشی در تاریخچه «تمدید» ثبت می‌شد نه
>    «فعال‌سازی». آن تاریخچه دقیقاً برای خوانده‌شدن در اختلاف مالی است.
>
> ✅ **بازیابی رمز با اجرای واقعی تأیید شد** (هفت مرحله، روی HTTP):
> درخواست ← ایمیل ← بررسی لینک ← رمز جدید ← استفاده‌ی دوباره رد شد (۴۰۰) ←
> ورود با رمز نو ۲۰۰ و با رمز قدیم ۴۰۱ ← ثبت‌نام بدون ایمیل ۴۰۰.
> دو تله هم بررسی شد: جواب ایمیل ناموجود **کاملاً یکسان** بود و ایمیلی نرفت؛
> و جواب تأیید **هیچ توکنی** نداشت.
>
> 🐛 **باگی که فقط اجرای واقعی گرفت:** `PANEL_URL` داخل `UPMARKET_PAYMENT` است
> نه `UPMARKET_AI` (چون اول callback درگاه لازمش داشت). من دیکشنری اشتباه را
> خواندم و **لینک ایمیل نسبی شد** — `/reset-password?...` بدون دامنه، یعنی
> لینکی که هیچ مشتری نمی‌تواند کلیک کند. هیچ تستی نمی‌گرفت چون همه‌شان فقط
> وجود `reset-password` در متن را چک می‌کردند. تستِ «لینک باید مطلق باشد» اضافه شد.
>
> ✅ **فاز ۱۹ کامل شد** — سه ادعای باقی‌مانده با اجرای واقعی روی HTTP آزمایش شدند،
> نه فقط با تست واحد:
> ۱. همکار با نقش «پشتیبانی»: چک‌لیست ۲۰۰، صورتحساب ۴۰۳، اعضا ۴۰۳، غریبه ۴۰۴.
> ۲. رد کردن خروجی → `REFUNDED` و مصرف از ۱ به ۰؛ تلاش چهارم رزرو `None` و
>    رایگان، کل زنجیره برگشت، **صفر** ردیف مصرف‌شده باقی ماند.
> ۳. چک‌لیست از ۱ به ۴ از ۸ رفت وقتی سه مورد واقعی پر شد، و قدم بعدی عوض شد.
>
> 🐛 **باگی که تست خودش گرفت:** تضمین کیفیت اول فقط **آخرین** تلاش را برمی‌گرداند.
> فروشگاهی که سه بار تلاش کرده و چیز قابل‌استفاده‌ای نگرفته، سه بار پول داده؛
> برگرداندن یکی از سه تا تضمین نیست، **تخفیف روی شکست** است. حالا کل زنجیره
> برمی‌گردد.
>
> ✅ **۴۹۹ تست بک‌اند سبز** (۲۶۳ → ۴۹۹ در این نشست).
...
- [x] **Job State Machine کامل** ✅ — `apps/jobs/machine.py`
  `QUEUED → PROCESSING → PREVIEW → WAITING_APPROVAL → APPROVED → RENDERING → QC → COMPLETED`
  - جدول است نه `if` پراکنده، و حرکت غیرمجاز **exception** می‌دهد. دو حرکتی که
    این جلویشان را می‌گیرد ارزش نام بردن دارند: `WAITING_APPROVAL → COMPLETED`
    یعنی پول گرفتن برای چیزی که کسی تأیید نکرده، و ورود دوباره به `RENDERING`
    یعنی دو بار خرج کردن دقیقه‌های GPU.
  - `RUNNING` قدیمی در enum ماند. ردیف‌های قبلی از آن استفاده می‌کنند و
    بازنویسی تاریخ برای خوشگل شدن دیاگرام، همان کاری است که توان توضیح دادن
    صورتحساب ماه قبل را از بین می‌برد.
  - `WAITING_APPROVAL` عمداً در `BUSY` **نیست**: کاری که منتظر آدم است کارت
    را نگه نداشته، و حساب کردنش به‌عنوان مشغول یعنی بی‌کار ماندن GPU تا وقتی
    صاحب فروشگاه از ناهار برگردد.
  - `staleness` هم اصلاح شد: کارِ منتظر تأیید **هرگز** به‌عنوان مرده جمع
    نمی‌شود. شب می‌ماند و صبح تأیید می‌شود؛ کشتنش یعنی دور ریختن ۵ ثانیه رندرِ
    تمام‌شده.
- [x] **تولید تدریجی با Preview بعد از هر قطعه** ✅
  - **آزمایش‌شده:** در حالت Safe بعد از قطعه‌ی اول، `client.generate` دقیقاً
    **یک بار** صدا زده شده و `concat` هیچ‌وقت — یعنی ۴۰ ثانیه‌ی بعدی GPU
    واقعاً مصرف نشده.
  - **صورت‌حساب به‌ازای هر قطعه، نه هر ویدیو.** رزرو یک‌جای کل ویدیو یعنی
    سهمیه‌ی فروشگاه تا صبح بلوکه می‌ماند، و فروشگاهی که ۵ ثانیه را می‌بیند و
    می‌رود، ۴۰ ثانیه رزرو کرده بود. حالا کارِ رهاشده فقط بابت چیزی که دیده
    پول داده — **آزمایش‌شده: ۵ ثانیه، نه ۱۵**.
- [x] **دو Mode: Safe و Auto** ✅ — پیش‌فرض **Safe** است، چون اولین ویدیوی یک
  فروشگاه‌دار باید بعد از ۵ ثانیه GPU غلط از آب دربیاید نه بعد از ۴۰.
  Auto یعنی «فقط وقتی خروجی بد است بایست»، پس به Quality Score نگاه می‌کند؛
  تا Vision QC فاز ۲۱ نیاید امتیازی وجود ندارد و Auto بی‌توقف می‌رود — و کد
  همین را می‌گوید، نه اینکه وانمود کند قضاوت کرده. حالت وسط کار هم عوض می‌شود،
  چون همان اولین پرامپت تأیید است که به صاحب فروشگاه می‌گوید بقیه را
  می‌خواهد ببیند یا نه.
- [x] **Cancel واقعی** ✅ — `apps/jobs/cancellation.py`
  لغو قبلی ردیف را عوض می‌کرد و اسپینر را می‌بست. از پنل عین همین به نظر
  می‌رسد؛ روی صورتحساب نه — **اعتبار خرج‌شده می‌ماند**. حالا چهار کار می‌شود:
  ورکر سر checkpoint بعدی می‌ایستد، فایل‌های نیمه‌کاره پاک می‌شوند، VRAM آزاد
  می‌شود، و **اعتبار برمی‌گردد**.
  - ترتیب مهم است: اعتبار **قبل** از حالت پایانی برمی‌گردد. اگر پروسه بین این
    دو بمیرد، کاری که در حالت غیرپایانی گیر کرده دیده و درست می‌شود؛ ولی کارِ
    CANCELLED که اعتبارش برنگشته، **تمام‌شده به نظر می‌رسد** و بی‌صدا غلط است.
  - قطعه‌های تمام‌شده پاک **نمی‌شوند**. کار لغوشده ممکن است سه کلیپ خوب داشته
    باشد و دور ریختنشان یعنی از بین بردن کاری که هنوز قابل تماشا است.
- [x] **Scene State ساختاریافته** ✅ — `services/video/scene_state.py`
  Wan از یک تصویر ادامه می‌دهد، و آن تصویر فقط چیزی را حمل می‌کند که در یک
  فریم دیده می‌شود. به همین دلیل ویدیوهای چسبانده‌شده drift می‌کنند: کاپشن در
  قطعه‌ی اول آبی بود، فریم آخر فقط آستین را نشان داد، و قطعه‌ی دوم یک رنگ
  اختراع کرد.
  - هشت محور: Characters/Objects/Location/Lighting/Camera/Clothing/Colors/Action
  - **سکوت یعنی «مثل قبل»، نه reset.** صحنه‌ای که چیزی از لباس نمی‌گوید یعنی
    همان لباس؛ همین یک تصمیم است که drift را می‌سازد یا جلویش را می‌گیرد.
  - `action` تنها فیلدی است که *قرار است* عوض شود؛ عوض شدن بقیه همان drift است.
  - جمله‌ها به شکل **قید** نوشته می‌شوند («the same X»، «still wearing Y») نه
    توصیف، چون بازگویی ساده برای مدل مثل معرفی یک سوژه‌ی جدید است و با خوشحالی
    نفر دومی با همان پالتو اضافه می‌کند.
  - `NEW_SCENE` هیچ چیز را منتقل نمی‌کند — کاتِ عمدی، drift نیست.
- [x] **Hardware Profiler** ✅ — `services/hardware.py`
  هر probe به `None` تنزل می‌کند نه به حدس. مدل ظرفیتی که عدد ساختگی VRAM
  بگیرد از مدلی که می‌گوید «نامشخص» بدتر است، چون نوع اول **شبیه جواب است**.
- [ ] Smart Scheduler (CPU/GPU per task) — پروفایلر هست؛ زمان‌بند بعد از اعداد فاز ۰.۵
- [ ] WebSocket/SSE به‌جای Polling
- [ ] **دو حالت تولید** ([۹.۱۴](#۹۱۴--دو-حالت-تولید-ساده-و-پیشرفته)) — ساده از داده‌ی موجود؛ پیشرفته تشخیص می‌دهد چه ورودی‌ای کم دارد و می‌پرسد
- [x] **دو نوع خروجی تصویر صریح و جدا** ✅ — از قبل ساخته شده بود:
  `GeneratedImage.Kind` سه مقدار دارد و `EDIT_DENOISE` برای هرکدام جدا است
  (`ENHANCED` ۰.۴۵ تا محصول واقعی حفظ شود، `POSTER` ۰.۷۵). متن فارسی پوستر با
  **کد** روی تصویر می‌نشیند (`services/imaging/text_overlay.py` با shaping
  راست‌به‌چپ و ارقام فارسی) — همان ایده‌ای که در `ایده‌ها.md` نوشته شده.
...
> ✅ **نیمه‌ی قابل‌اندازه‌گیری DoD تأیید شد** — «بدون اینکه GPU برای ۴۰ ثانیه‌ی
> بعدی مصرف شده باشد» یک ادعای آزمون‌پذیر است و آزمون دارد:
> `test_safe_mode_stops_after_the_first_segment` بررسی می‌کند که بعد از قطعه‌ی
> اول، کلاینت رندر **یک بار** صدا زده شده و `concat` هیچ‌وقت.
> نیمه‌ی دیگر («زیر یک دقیقه») به سرعت واقعی کارت بستگی دارد و **فاز ۰.۵** است.
>
> 🐛 **ماشین حالت، اولین باگ خودش را روی کد خودم گرفت:** تسک ویدیو از
> `PROCESSING` مستقیم به `WAITING_APPROVAL` می‌رفت و ماشین ردش کرد. درست هم
> کرد — `PREVIEW` یک حالت واقعی است: لحظه‌ای که پیش‌نمایش موجود است و در حال
> قضاوت شدن. در Safe فوراً به `WAITING_APPROVAL` می‌رود، در Auto به
> `PROCESSING` برمی‌گردد. حالا کد مسیر رودمپ را واقعاً طی می‌کند.
>
> ✅ **۴۹۹ تست بک‌اند سبز** (۲۶۳ → ۴۹۹ در این نشست).
...
- [ ] **زمان‌بندی و Automation محتوا** — زمان‌بندی پست، برنامه‌ریزی انتشار، تولید خودکار دوره‌ای
- [ ] **کمپین به‌عنوان مدیر وظایف** — `Campaign → Marketing Tasks · Advertising Tasks · Content Tasks · Sales Tasks`، از ایده و برنامه‌ریزی تا اجرا و وضعیت هر کار

**DoD:** دو کانال واقعی وصل شوند و هر دو از یک Inbox مدیریت شوند. یک پست زمان‌بندی‌شده در ساعت مقرر **بدون دخالت انسان** منتشر شود.
...
### فاز ۲۷ — سایت عمومی کامل

مرجع: [۹.۱۸](#۹۱۸--سایت-عمومی) · [`../../سایت عمومی.md`](../../سایت%20عمومی.md)

> **وضعیت ۲۰۲۶/۰۹/۱۲:** سایت دوزبانه شد و از ۱۱ روت به **۳۳ روت پایه** رسید
> (۱۰۷ صفحه‌ی تولیدشده، ۹۹ URL در sitemap). سنجش، ابزار رایگان و مقایسه‌ها وصل شدند.
> مانده فقط چیزهایی که **بلوکر بیرونی** دارند: سه صفحه منتظر اولین مشتری واقعی،
> `/resources` منتظر محتوای نوشته‌شده، و `/login`+`/register` وابسته به درگاه پرداخت فاز ۱۹.

- [~] **۲۷.۱ — هسته‌ی بازاریابی:** ✅ `/solutions` · `/use-cases`
  - ⛔ **`/case-studies` · `/customers` · `/testimonials` عمداً ساخته نشدند.** هنوز
    هیچ مشتری واقعی نداریم (`data.json` هم مشتری و نظر ندارد)، و خودِ
    [`سایت عمومی.md`](../../سایت%20عمومی.md) بند ۳۴ می‌گوید «نمونه‌کار ساختگی را
    به‌عنوان مشتری واقعی نمایش ندهیم». ساختن این سه صفحه با محتوای جعلی، یا
    صفحه‌ی خالی «هنوز مشتری نداریم»، هر دو بدتر از نداشتنشان‌اند.
    **این بلوکر کسب‌وکاری است، نه مهندسی — با اولین مشتری واقعی باز می‌شود.**
- [~] **۲۷.۲ — تمپلیت‌های سئو:** ✅ `/solutions/[slug]` (۴) · `/use-cases/[slug]` (۶) ·
  ✅ **`/features/[slug]` (۷ صفحه × ۲ زبان = ۱۴)** — هر قابلیت با ساختار کامل
  مشخصات: مشکل ← چه می‌کند ← چطور ← نمونه ← چرا مهم است ← CTA.
  استودیو عمداً به **دو** صفحه شکست (`ai-image-generation` و `ai-video-generation`)
  چون «ساخت پوستر» و «ساخت ویدیو» دو جستجوی متفاوت‌اند و یک صفحه نمی‌تواند
  صادقانه برای هر دو رتبه بگیرد. قابلیت‌های ساخته‌نشده در بخش جدای
  «هنوز ساخته نشده» با برچسب می‌آیند، نه قاطی چیزی که امروز کار می‌کند.
  - مانده: `/blog/category/[slug]` · `/case-studies/[slug]` (وابسته به بلوکر ۲۷.۱)
- [x] **۲۷.۳ — دوزبانه:** ✅ فارسی در ریشه (`/pricing`)، انگلیسی با پیشوند (`/en/pricing`) — با middleware rewrite، پس URLهای فارسی تمیز ماندند و چیزی نشکست. `hreflang` + `x-default` + `canonical` روی همه، sitemap دوزبانه (۶۹ URL)، `lang`/`dir` از بایت اول درست. سوییچ زبان کاربر را روی همان صفحه نگه می‌دارد. **ترجمه نوشته شده، نه ماشینی.**
  - مقاله‌های وبلاگ عمداً فقط فارسی‌اند و `hreflang`شان فقط `fa` را اعلام می‌کند — نه آدرسی که ۴۰۴ بدهد.
  - ✅ `/en/showcase` و `/en/contact` هم انگلیسی شدند (فرم تماس هم دوزبانه شد).
  - `/en/blog` انگلیسی است ولی صریح می‌گوید مقاله‌ها فارسی‌اند، و به آدرس فارسی
    مقاله لینک می‌دهد — نه آدرس انگلیسی‌ای که وجود ندارد.
- [~] **۲۷.۴ — اتصال واقعی:** ✅ فرم تماس واقعی متصل به `POST /api/v1/leads/` با ثبت Lead، هانی‌پات و throttle اختصاصی (۵/ساعت) · ✅ `?plan=` تا ساخت فروشگاه حفظ و اعمال می‌شود — مانده: `/login` و `/register` روی خود سایت، و بازیابی رمز
- [x] **۲۷.۵ — حقوقی و سیستمی:** ✅ هر پنج صفحه ساخته شد (`lib/legal.ts` + یک رندرر مشترک) — **با هشدار صریح «بازبینی حقوقی نشده» روی هرکدام**. ۴۰۴ اختصاصی از قبل بود.
- [~] **۲۷.۶ — رشد:** ✅ `/tools` + ۳ ابزار · ✅ `/compare` + ۳ مقایسه (هر کدام × ۲ زبان)
  - **ابزارها به مدل وصل نیستند — عمداً.** GPU گلوگاه کل مدل مالی است و مدل متنی
    هم به‌ازای هر فراخوانی پول دارد؛ دادن تولید ناشناس و نامحدود به غریبه‌ها
    دقیقاً همان منبعی را می‌سوزاند که محصول رویش می‌فروشد، و سوءاستفاده‌اش هم ساده است.
    پس ابزارها **در مرورگر** اجرا می‌شوند و هر صفحه صریح می‌گوید هوش مصنوعی نیست.
    آن‌وقت upsell یک تفاوت **واقعی** است (عکس واقعی محصول + کاتالوگ خودت)،
    نه نسخه‌ی فلج‌شده‌ی همان چیز.
  - سه ابزار ساخته شد نه هفت‌تای فهرست مشخصات: کپشن‌ساز، بایوساز، تقویم محتوا.
    خروجی هر سه باید چیزی باشد که آدم واقعاً پست کند — ابزاری که متن پرکننده بدهد
    از نبودنش بدتر است.
  - مقایسه‌ها **منصفانه‌اند**: هر صفحه بخش «کِی گزینه‌ی دیگر بهتر است» دارد و
    **اول** می‌آید، نه زیر نیمه‌ی فروش. صفحه‌ی «خودم انجام بدهم» صریح می‌گوید
    خیلی فروشگاه‌ها به هیچ ابزاری نیاز ندارند.
  - `tool_used` به سنجش فاز ۲۷.۷ وصل است، پس معلوم می‌شود کدام ابزار واقعاً جواب می‌دهد.
  - ⛔ **`/resources` ساخته نشد.** راهنما، چک‌لیست و ebook یعنی محتوای نوشته‌شده که
    وجود ندارد؛ صفحه‌ی منابعِ خالی همان مشکل صفحه‌ی مشتریِ خالی است. وقتی محتوا
    نوشته شد، تمپلیتش کوچک است.
- [x] **۲۷.۷ — سنجش:** ✅ سنجش **first-party** — رویدادها در دیتابیس خودمان، نه گوگل آنالیتیکس.
  - **چرا first-party:** سیاست کوکی همین سایت می‌گوید کوکی ردیابی شخص ثالث نمی‌گذاریم و
    صفحه‌ی حریم خصوصی می‌گوید پردازش روی سرور خودمان است. چسباندن اسکریپت شخص ثالث،
    هر دو را دروغ می‌کرد — و آن دو جمله، خودِ فروش محصول‌اند نه متن تشریفاتی.
  - **بدون کوکی:** یک شناسه‌ی تصادفی در `sessionStorage` که با بستن تب می‌رود.
    برای دوختن یک بازدید به قیف کافی است، برای دنبال‌کردن کسی بی‌فایده.
  - ۸ رویداد: `page_view` · `cta_click` · `pricing_view` · `plan_selected` ·
    `contact_submitted` · `signup_started` · `signup_completed` · `tool_used`
  - **قیف session-محور** (نه شمارش رویداد) در جنگو ادمین + انتساب کانال + پربازدیدترین صفحه‌ها.
  - کلیک CTA با یک listener واحد گرفته می‌شود، نه `onClick` روی هر دکمه — وگرنه هر
    دکمه‌ی جدیدی که یادمان برود، دقیقاً همانی است که بعداً عددش را لازم داریم.
  - پنل مبدأ متفاوتی دارد، پس شناسه‌ی بازدید با `?s=` دست‌به‌دست می‌شود؛ بدون آن
    «دیدن قیمت» هرگز به «ثبت‌نام» وصل نمی‌شد.
  - ✅ سیاست حریم خصوصی و کوکی (هر دو زبان) به‌روز شد تا این رفتار را **راست** بگوید.
  - **۲۵ تست** برای ingest و قیف.

> **🐛 دو باگ ابزارها که فقط با اجرای واقعی پیدا شدند:**
> ۱. در کپشن فارسی `undefined` چاپ می‌شد — شیفت علامت‌دار روی seed بزرگ منفی می‌شد
>    و `منفی % طول` از ابتدای آرایه رد می‌کرد.
> ۲. خطوط خالیِ عمدیِ کپشن بلند حذف شده بودند: موقع رفع یک خطای تایپ،
>    `.filter(Boolean)` گذاشتم که `""` را هم دور می‌ریخت و کل کپشن یک بلوک چسبیده شد.
>    هیچ‌کدام در تایپ‌چک یا بیلد دیده نمی‌شدند.

> **🐛 دو نقصی که تست‌ها گرفتند:**
> ۱. قیف ادعا می‌کرد هر مرحله زیرمجموعه‌ی قبلی است ولی کد این کار را نمی‌کرد.
>    مهم‌تر: «انتخاب پلن» **واقعاً اختیاری** است (دکمه‌ی ثبت‌نام بیشتر صفحه‌ها پلن ندارد)،
>    پس زنجیره‌ی سخت، ثبت‌نام‌های واقعی را دور می‌ریخت و صفر گزارش می‌کرد. حالا مرحله‌های
>    اختیاری علامت دارند و افت نسبت به آخرین مرحله‌ی **الزامی** سنجیده می‌شود.
> ۲. یک بازدید زیر **دو** کانال شمرده می‌شد (هم `instagram` هم «مستقیم»)، چون هر
>    `page_view` با utm خودش گروه می‌شد. حالا هر بازدید به **اولین** منبعی که دیده
>    نسبت داده می‌شود، پس جمع کانال‌ها با تعداد بازدید واقعی می‌خواند.

> **🐛 نقصی که حین کار پیدا و رفع شد:** `SectionHead` همیشه `h2` می‌ساخت، پس
> **بیشتر صفحه‌های سایت اصلاً `h1` نداشتند** — از جمله `/features`، `/pricing`،
> `/showcase` و `/blog`. برای سایتی که کل معماری‌اش روی سئو بسته شده این نقص
> جدی بود. حالا `SectionHead` پراپ `as` دارد و هر صفحه دقیقاً یک `h1` دارد
> (۱۹ صفحه بررسی شد).

**DoD:** کاربر از گوگل وارد یک صفحه‌ی سئو می‌شود، پلن انتخاب می‌کند، ثبت‌نام می‌کند و **همان پلن** در پنل فعال است. Core Web Vitals بررسی شده. صفحه‌های خصوصی ایندکس نمی‌شوند. هیچ صفحه‌ای صرفاً برای پر کردن sitemap ساخته نشده.

> ⚠️ فاز ۲۷.۴ به درگاه پرداخت فاز ۱۹ وابسته است. بقیه‌اش مستقل است و می‌تواند زودتر برود.

---

### فاز ۲۸ — هوش فروشگاه

مرجع: [۹.۱۲](#۹۱۲--هوش-فروشگاه-فروش-موجودی-و-سود-واقعی) · [۹.۱۳](#۹۱۳--ترندها-و-اتصالشان-به-تولید-محتوا)

- [ ] افزودن **قیمت تمام‌شده** به مدل محصول — بدون آن هیچ سودی محاسبه‌شدنی نیست
- [ ] تحلیل فروش و موجودی: نمودار فروش، نمودار موجودی، گردش
- [ ] تفکیک **پرفروش‌ترین** از **سودآورترین** — این دو معمولاً یکی نیستند
- [ ] مقایسه‌ی محصولات کنار هم
- [ ] جمع‌آوری ترند ایرانی و خارجی با **منبع و تاریخ** — نه ترند ساختگی مدل
- [ ] پیشنهاد محتوا از ترکیب ترند + فروشگاه + محصول + مخاطب
- [ ] اتصال مستقیم: انتخاب ترند → انتخاب محصول → صفحه‌ی تولید محتوا
- [ ] تغذیه‌ی داده‌ی فروش به ایجنت فروش و کمپین

**DoD:** فروشگاه‌دار می‌تواند بگوید کدام محصولش واقعاً سود می‌دهد، و از یک ترند واقعی با یک کلیک به تولید محتوا برسد.

---

### فاز ۲۹ — پلتفرم API عمومی

مرجع: [۹.۱۶](#۹۱۶--پلتفرم-api-عمومی)

- [ ] احراز هویت API (کلید، چرخش کلید، ابطال)
- [ ] سهمیه و محدودیت نرخ، متصل به همان سیستم اعتبار فاز ۱۹
- [ ] پنج API: Sales Admin · Text & Caption · Advertising Video · Advertising Poster · Product Showcase
- [ ] مستندات + نمونه‌کد + محیط تست
- [ ] صفحه‌ی API در پنل: کلیدها، مصرف، لاگ درخواست‌ها

**DoD:** یک توسعه‌دهنده‌ی بیرونی بدون کمک ما، با مستندات، یک پوستر تولید می‌کند و مصرفش در پنل دیده می‌شود.

---

### فاز ۳۰ — بازاریابی تلفنی (Coming Soon)

مرجع: [۹.۱۷](#۹۱۷--بازاریابی-تلفنی)

> **این فاز تا حل دو قید حقوقی شروع نمی‌شود.** کد نوشته نمی‌شود؛ فقط معماری رزرو است.

- [ ] **پیش‌شرط:** روشن شدن قانونی بودن ضبط تماس و سازوکار رضایت
- [ ] **پیش‌شرط:** سیاست جمع‌آوری و نگه‌داری اطلاعات شخصی
- [ ] Customer Research و Company Research
- [ ] ارتباط اولیه (پیام/تیکت)
- [ ] Mobile Call Assistant
- [ ] Real-Time Sales Assistant
- [ ] Call Recording & Analysis
- [ ] Post-Call Analysis → به‌روزرسانی پروفایل مشتری

**DoD:** تا وقتی دو پیش‌شرط بالا مکتوب حل نشده‌اند، این فاز DoD ندارد و **نباید در سایت یا به مشتری وعده داده شود**.
...
```text
     ...
     ┌───────────────┼───────────────┐
     ▼               ▼               ▼
مسیر زیرساخت     مسیر کیفیت      مسیر بازار
۱۸ Production   ۲۰ Pipeline     ۲۷ سایت عمومی
۱۹ Core         ۲۱ Quality      ۲۸ هوش فروشگاه
                ۲۲ Audio
     └───────────────┼───────────────┘
                     ▼
              ۲۳ Editing Engine
              ...
              ۲۹ API Platform
                 ...
                 ۲۶ GTM
                     ▼
         ۳۰ بازاریابی تلفنی (معلق)
```

**سه مسیر بعد از فاز ۰.۵ موازی‌اند** و به هم وابسته نیستند — می‌شود هم‌زمان رویشان کار کرد.

وابستگی‌های واقعی:

| فاز | به چه وابسته | چرا |
|---|---|---|
| ۲۳ Editing | ۲۱ Quality | تعمیر سه‌سطحی از همان ابزارهای QC استفاده می‌کند |
| ۲۵ Learning | ۲۴ Omnichannel | بدون اتصال پلتفرم، داده‌ی عملکرد برنمی‌گردد |
| ۲۷.۴ فرم ثبت‌نام | ۱۹ پرداخت | مسیر `?plan=` بدون درگاه ناقص می‌ماند |
| ۲۸ ترند→محتوا | ۲۰ Pipeline | جایی باید محتوا تولید شود |
| ۲۹ API | ۱۹ اعتبار | مصرف API باید از همان سهمیه کم شود |
| ۳۰ تلفنی | — | **معلق تا حل دو قید حقوقی** |

بقیه‌ی فاز ۲۷ (سایت عمومی) به هیچ‌کدام وابسته نیست و **می‌تواند از همین حالا شروع شود**.
